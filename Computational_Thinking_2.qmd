...
author: "Seleonalela Hunter - Selesitino"
format: pdf
editor: visual
---
# Overview
...
if(x > 10) # if() creates the CONDITION
{ 
  # Paste takes the value stored in x and combines that with a character string
  print(paste(x, "is greater than 10")) 
} else # else() creates the alternative output if CONDITION is not met.
{ 
...
#### Q1.1 Answer:
#### The output of the if else statement when x = 11 instead of 5 is "is greater than 10."
...
```{r}
x <- 5 # I had to change the "five" back to "5" 
       # or else I could not Render my pdf.
...
#Recieved these additional errors, so traceback was unable to run.
...
# define a variable
x <- 11
# when x = 10, output = "x is 10"
# when x = 9, output = "x is less than 10"
...
```{r}
vec <- c(9, 10, 11, 12) # created vector using c()
# named the new vector using "vec" using  <-
```
...
for (i in 1:length(vec)) { # run the condition starting from first value to last value in vector.
...
```{r}
# define Vector:
y <- c(-2, 42, 0, 10) # creating vector y
...
#### Q1.3 Answer: Niwot Ridge measured stress by observing the concentration of  glucocorticoid metabolite present in the fecal samples collected from individual pika's. The stress vairable is called "concentration_pg_m" and is measured in picograms GCM/gram.
...
#### Q1.4 Answer: A row in the dataset represents 1 observation for a single pika. 
...
nwt_pikas_summerstress <- nwt_pikas_categ2 %>% 
  mutate(summer_stress_category = case_when(
    (month == 6 | month == 7) & concentration_pg_g > 5000 ~ "Early summer stress",# In June or July if GCM concentration is > 5000 print Early Summer Stress
    (month == 6 | month == 7) & concentration_pg_g <= 5000 ~ "Early summer chill", # In June or July if GCM concentration is < 5000 print Early Summer Chill
...
#### Q2.1 Answer: I am using the knz_bison dataset.
...
# creating column for bison age + changing weight to from units of Ib to kg:
new_bison <- knz_bison %>% # we are saving this new dataframe as new_bison.
mutate(bison_age = rec_year - animal_yob, # this code gives us a new column with the age of the bison
weight_kg = animal_weight*0.453592) # this code creates the new column with the weight


# Creating new column with only Female bison:
new_bison$female <- NA #creating a new column for 'female' and filling it with NA's
new_bison$female[new_bison$animal_sex == "F"] <- "F" # filling new column with F if it is true.


# Filtering out any Observations of non-Female bison:
new_bisonF <- new_bison %>% # creating new data.frame for output
  filter(!is.na(female)) # filtering out any row with NA

head(new_bisonF)



unique(new_bisonF$bison_age) # shows how many unique ages are in columnn.
```
...
#### Q2.2 Answer:
#### Using a for loop with an if else statement I want to loop through it the bison_age column, in the tidied version of the knz_bison data set I created. This will let me see which female bison are in their years of high fertility, and which ones are at a stage where they experience reduced fertility because they are either immature or advanced in age. 
...
```{r}
ba <- new_bisonF$bison_age #creating a vector for bison age
fertilities <- c()
# telling R to run this for the length of the ba vector.
for (i in 1:length(ba)) { 
  # tells R to ignore all NA's in the following if/else() statements
  if (is.na(ba[i])) {
    next
  }
  if(ba[i] >= 2 & ba[i] <= 9) {
      # if the bison is 2:9 R will print "highly fertile"
      fertilities[i] <- "Highly Fertile"
    } else {
      # if bison is less than 2 years old R will Print "immature reduced fertility"
      if(ba[i] < 2){
        fertilities[i] <- "Immature Reduced Fertility"
      } else {
      # if bison is not less than 2 or 2:9 years but older, R will print "older reduced fertility
        fertilities[i] <- "Older Reduced Fertility"
    }
    }}

# Check out the first 6 values of the vector
head(fertilities)
...
#### Q2.4 Answer: With the if else/case_when() functions I want to create a new column called bison fertility. This column will contain values that will tell me, based off of age, which bison (in each of the 3 groups I defined in the for loop if else statements) are at an age where they are highly fertile, or at an age where they experience reduced fertility due to being too young or old. This will allow me to make a skatter plot relating fertility and age to weight.
  
------

### Q2.5 Apply an if else or case_when to this dataset

Apply an if else or case_when in some way to this data set. Describe what you are doing in comments on each line.

#### Q2.5 Answer: 

```{r}
new_bisonF1 <- new_bisonF %>% # naming output of this code 
  # Call the new column bison_fertility
  mutate(bison_fertility = case_when(
    # bison < 2 years old will have a value in the new column = "Immature Reduced Fertility"
# bison < 2 years will be recorded as "Immature Reduced Fertility"
    bison_age < 2 ~ "Immature Reduced Fertility",
    # bison_age == 2 ~ "Highly Fertile",
    # bison_age == 3 ~ "Highly Fertile",
    # bison_age == 4 ~ "Highly Fertile",
    # bison_age == 5 ~ "Highly Fertile",
    # bison_age == 6 ~ "Highly Fertile",
    # bison_age == 7 ~ "Highly Fertile",
    # bison_age == 8 ~ "Highly Fertile",
    # bison_age == 9 ~ "Highly Fertile",
# bison between 2 and 9 years old will be recorded as "Highly Fertile"
    bison_age >= 2 & bison_age <= 9 ~ "Highly Fertile",
# bison older than 9 years will be recorded as " Older Reduced Fertility"
    bison_age > 9 ~ "Older Reduced Fertility",
    .default = "Unknown Age" # any age that is not listed will be inputed as "Unknown age".
  ))
 
head(new_bisonF1)# checking to see column was made correctly in new data.frame.
```

#### Graph:

I wanted to make some graphs to help compare fertility, age, and weight, to see if there are any noticeable patterns/trends.  

```{r}
Female_Bison_Plot1 <- ggplot(data = new_bisonF1, aes(x=bison_fertility, y=weight_kg))+
geom_jitter(aes(color=bison_age), alpha=0.5)+
labs(x="Fertility", y="Weight (kg)", title = "Wieght Distribution of Reproductively Active Female Bison", color = "Bison Age")
...
```{r}
# for this graph I did use Chat GPT 
# to help with finding what geom_plot could more effectively 
# represented the patterns I saw in the first graph.

ggplot(data = new_bisonF1, aes(x = bison_fertility, y = animal_weight)) +
  geom_violin(fill = "grey", color = NA) +
  geom_jitter(aes(color = bison_age), width = 0.09, alpha = 0.7) +
  scale_color_viridis_c() +
  theme_minimal()+
  labs(x="Fertility", y="Weight (kg)", title = "Weight Distribution of Reproductively Active Female Bison", color = "Bison Age")
```

```{r}
Female_Bison_Plot2 <- ggplot(data = new_bisonF1, aes(x=bison_fertility, y=weight_kg))+
geom_boxplot(aes(color=bison_fertility), alpha=0.5)+
labs(x="Fertility", y="Weight (kg)", title = "Wieght Distribution of Reproductively Active Female Bison", color = "Bison Age")
